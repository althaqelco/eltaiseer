...
export default function PropertyPageClient({
  ...
  // Load property (only when the server did not provide it) and related properties
  useEffect(() => {
    const loadProperty = async () => {
      try {
        let prop = initialProperty;
        if (!prop) {
          setIsLoading(true);
  ...
  // Check favorite status (المخزن الموحد eltaiseer_favorites — كانت الصفحة تستخدم مفتاحاً منفصلاً)
  useEffect(() => {
    if (property) {
  ...
  const toggleFavorite = () => {
    if (!property) return;
    const newState = toggleFavoriteStore(property.id);
    if (newState) {
